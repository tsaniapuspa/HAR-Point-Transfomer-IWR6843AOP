# Standard Imports
from PySide2 import QtCore   # ← tambahan
...
class UARTParser():
    def __init__(self, type):
        ...
        self.filepath = datetime.now().strftime("%m_%d_%Y_%H_%M_%S")
        ...
        if (self.saveBinary == 1):
            ...
            if (self.uartCounter % self.framesPerFile == 0):
                ...
                with open('./binData/'+self.filepath+'/replay_' + str(math.floor(self.uartCounter/self.framesPerFile)) + '.json', 'w') as fp:
                    ...
                    pass
        # debug metadata
        outputDict['_parser_ts'] = time.time()
        outputDict['_uart_counter'] = getattr(self, 'uartCounter', None)
    ...
    def connectComPort(self, cliCom, cliBaud=115200):
        ...
        self.cliCom = serial.Serial(
            cliCom, cliBaud, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=.6)
    ...
    def sendCfg(self, cfg):
        ...
        for line in cfg:
            ...
            # The baudrate CLI line changes the CLI baud rate on the next cfg line to enable greater data streaming off the xWRL device.
    ...
    def convertFrameDataToDataFrame_pengambilan(self, frameData):
        rows = []
        ...
        for idx, point in enumerate(pointCloud, start=1):
            ...
            rows.append([
                timestamp, idx, x, y, z, doppler, snr,
                range_target, azimuth_deg, elevation_deg
            ])

        df = pd.DataFrame(rows, columns=[
            'timestamp','numFrame', 'x', 'y', 'z',
            'doppler', 'SNR', 'Range', 'Azimuth', 'Elevation'
        ])
        return df
...
class UartWorker(QtCore.QObject):
    ...
    def run(self):
        while self.running:
            if self.parser.parserType == "DoubleCOMPort":
                out = self.parser.readAndParseUartDoubleCOMPort()
            else:
                out = self.parser.readAndParseUartSingleCOMPort()
