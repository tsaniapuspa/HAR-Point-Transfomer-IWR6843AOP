...
def parsePointCloudTLV(tlvData, tlvLength, outputDict):
    ...
    pointStruct = '5f'  # X, Y, Z, and Doppler
    pointStructSize = struct.calcsize(pointStruct)
    numPoints = int(tlvLength/pointStructSize)

    for i in range(numPoints):
        try:
            x, y, z, doppler, snr  = struct.unpack(pointStruct, tlvData[:pointStructSize])
        except:
            numPoints = i
...
def parseSphericalPointCloudTLV(tlvData, tlvLength, outputDict):
    ...
    # --------------------------------------------------------
    # 🔍 DEBUG B: LOG SPHERICAL SEBELUM KONVERSI
    # --------------------------------------------------------
    print("\n--- [DEBUG B] Spherical (raw dari TLV, sebelum cartesian) ---")
    print("Range    =", pointCloud[:10,0])
    print("Azimuth  =", pointCloud[:10,1])
    print("Elevation=", pointCloud[:10,2])
    print("Doppler  =", pointCloud[:10,3])
